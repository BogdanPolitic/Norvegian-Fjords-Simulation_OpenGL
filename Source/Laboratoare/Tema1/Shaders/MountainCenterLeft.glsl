...
void makeMountainPrimitive(int instance, int i, bool left_side, bool inferior, float curve_step, float height, float prev_elev, float curr_elev, float width) {
	if (left_side && inferior) {
		texCoord = vec2(1, 0);
		gl_Position = Projection * View * vec4(vec2To3(point_left(instance + 1, i + 1, curve_step, branch_width), height), 1.f);EmitVertex();
		texCoord = vec2(1, 1);
		gl_Position = Projection * View * vec4(vec2To3(point_left(instance + 1, i, curve_step, branch_width), height), 1.f);	EmitVertex();
		texCoord = vec2(0, 1);
	...
	if (instance[0] < no_inst)
	{
		...
		for (int i = 0; i < no_gen; i++) {
			mountain_crease[i] = (point_front(-1, i + 1, curve_step, branch_width)
									+ point_left(1, i + 1, curve_step, branch_width))
									/ 2.f;
		}
