...
void makeMountainPrimitive(bool is_corner, int instance, int i, bool is_main, bool inferior, float curve_step, float height, float width) {
	if (is_corner) {
		texCoord = vec2(0, 1);
		gl_Position = Projection * View * vec4(vec2To3(mountain_right[0], height + mountain_height), 1.f);						EmitVertex();
		texCoord = vec2(1, 0);
		gl_Position = Projection * View * vec4(vec2To3(point_main(instance + 1, no_gen - 2, curve_step, 1.f), height), 1.f);	EmitVertex();
		texCoord = vec2(1, 1);
		gl_Position = Projection * View * vec4(vec2To3(point_right(instance - 1, 1, curve_step, width), height), 1.f);				EmitVertex();
		EndPrimitive();
		return;
	}
	...
	if (!is_main && inferior) {
		texCoord = vec2(0, 0);
		gl_Position = Projection * View * vec4(vec2To3(mountain_right[i - 1], height + mountain_height), 1.f);								EmitVertex();
		texCoord = vec2(0, 1);
		gl_Position = Projection * View * vec4(vec2To3(point_right(instance - 1, i, curve_step, width), height), 1.f);	EmitVertex();
		texCoord = vec2(1, 0);
		gl_Position = Projection * View * vec4(vec2To3(mountain_right[i], height + mountain_height), 1.f);									EmitVertex();
	...
	if (!is_main && !inferior) {
		...
		gl_Position = Projection * View * vec4(vec2To3(point_right(instance - 1, i, curve_step, width), height), 1.f);	EmitVertex();
		texCoord = vec2(1, 1);
		gl_Position = Projection * View * vec4(vec2To3(point_right(instance - 1, i + 1, curve_step, width), height), 1.f);									EmitVertex();
...
void calculateLowAndHighXPoint(float curve_step) {
	...
	low_x_point = minX(equidistant_points[1], equidistant_points[1]);	// doar cel "din dreapta" ne intereseaza in urma apelului de mai sus
	calculateEdgePoints(vec3To2(bezier_front(0)), 
						...
						false, 
						false);
	high_x_point = maxX(equidistant_points[0], equidistant_points[1]);
}
...
void main()
{
	if (instance[0] < no_inst)
	{
		...
		vec3 common_point = middleOfSegment(point_main(1, no_gen - 3, curve_step, 1.f + mountain_width),
											point_right(-1, 1, curve_step, branch_width + mountain_width),
											1.f);
		...
		// calculate mountain_right:
		mountain_right[0] = vec3To2(common_point);
		for (int i = 1; i < no_gen; i++) {
			mountain_right[i] = point_right(-1, i + 1, curve_step, branch_width + mountain_width);
		}
